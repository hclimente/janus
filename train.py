...
if args["pretrain"]:
    ...
    net = CSN(feature_extractor=vgg19, p_dropout=args["dropout"]).to(device)
else:
    net = CSN(p_dropout=args["dropout"]).to(device)
...
pd.DataFrame({"train_loss": tr_losses, "test_loss": te_losses}).to_csv(
    "csn_%s_pretrain_%s_dropout_%s_margin_%s_seed_%s.tsv"
