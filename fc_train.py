...
if __name__ == "__main__":
    ...
    net = FCSN(p_dropout=args["dropout"]).to(device)
    ...
    with trange(args["epochs"]) as epochs:
        for epoch in epochs:
            ...
            if epoch % 10 == 0:
                torch.save(
                    net.state_dict(),
                    "fcsn_%s_dropout_%s_margin_%s_seed_%s_epoch_%03d.torch"
                    % (
                        args["split"],
                        args["dropout"],
                        args["margin"],
    ...
    pd.DataFrame({"train_loss": tr_losses, "test_loss": te_losses}).to_csv(
        "janus_%s_dropout_%s_margin_%s_seed_%s.tsv"
        % (args["split"], args["dropout"], args["margin"], args["seed"]),
        sep="\t",
        index=False,
    )
